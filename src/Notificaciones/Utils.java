...
public class Utils {
    ...
    static{
        String str=getProperty("os.name").toLowerCase();
    ...
    public static Rectangle getScreenSize(){
        if(screen==null){
            if(getOperativeSystem()<10){//A little hack for Windows
            ...
            out.println("Current workspace: "+screen.width+"x"+screen.height+"px");
        }
        return screen;
    }
    ...
    public static boolean isTranslucencySupported(){
        ...
        if(getProperty("java.version").indexOf("1.6")!=-1){
            err.println("Per-pixel translucency is currently not supported.\nPlease upgrade your platform to Java7 to support this feature.");
            nativeTrans = false;
        }else{
            if (!gd.isWindowTranslucencySupported(PERPIXEL_TRANSLUCENT)) {
                err.println("Error while starting: Per-pixel translucency is not supported.");
                nativeTrans = false;
