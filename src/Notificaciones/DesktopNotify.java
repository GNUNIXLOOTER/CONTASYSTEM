...
import static Notificaciones.DesktopNotifyDriver.getFontMetrics;
import static Notificaciones.DesktopNotifyDriver.postPane;
...
public class DesktopNotify {
    ...
    private static final Logger LOG = getLogger(DesktopNotify.class.getName());
    
    static{
        defaultIcons=new Image[8];
        for(int i=0;i<defaultIcons.length;i++){
            defaultIcons[i]=new ImageIcon(DesktopNotify.class.getResource("/Notificaciones/"+(i+1)+".png")).getImage();
        }
    }
    ...
    protected long expTime(){
        return timeOut==0? MAX_VALUE:popupStart+timeOut;
    }
    ...
    public void render(int x, int y, boolean hover, Graphics2D rd, long l){
        ...
        rd.setRenderingHint(KEY_TEXT_ANTIALIASING, VALUE_TEXT_ANTIALIAS_ON);
...
//            System.out.println(d);
            ...
            rd.scale(d, d);
            rd.setComposite(getInstance(SRC_OVER,(float)d));
        }
...
//        }
        ...
        rd.setComposite(getInstance(SRC_OVER));
    }
    ...
    protected void sortMessage(){
        if(!title.isEmpty()) {
            tlts=splitLines(title, new Font("Verdana", BOLD,14));
        }
        if(!message.isEmpty()) {
            msgs=splitLines(message, new Font("Verdana", PLAIN,12));
        }
    ...
    private String[] splitLines(String in, Font font){
        ...
        FontMetrics ftm = getFontMetrics(font);
        for(String str:strs){
