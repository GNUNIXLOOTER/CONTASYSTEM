...
import static Notificaciones.Utils.getBackgroundCap;
import static Notificaciones.Utils.getScreenSize;
import static Notificaciones.Utils.isTranslucencySupported;
...
public class DesktopNotifyDriver {
    ...
    private static void sparkControlThread(){
        if(tredo==null){
            tredo=new Thread(new Runnable(){@Override public void run(){
                    out.println("Desktop Notify Driver Thread started.");
                frame.finished=false;
                while(!frame.finished){
                    frame.repaint();
                    try{
                            sleep(20);//FPS> 10:Super-high, 20: High, 40: Normal, 80: Low
                    }catch(InterruptedException ex){}
                }
                frame.dispose();
                tredo=null;
                    out.println("Desktop Notify Driver Thread finished.");
            }},"");
            tredo.start();
        }
    }
    ...
    static class DesktopLayoutFrame extends JDialog{
        ...
        DesktopLayoutFrame(){
            ...
            setUndecorated(true);
            nativeTrans=isTranslucencySupported();
            ...
            setAlwaysOnTop(true);
            setVisible(true);
            out.println("Desktop Notify Frame deployed.");
        }
        ...
        public void setVisible(boolean visible){
            ...
            if(visible && !bool && !nativeTrans) bg=getBackgroundCap(getBounds());
        }
                    ...
                    if(y>0){
                        ...
                        if(p!=null){
                            if(p.y>y && p.y<y+window.h){
                                hover=true;
                                if(window.getAction()!=null) cur=HAND_CURSOR;
                                if(clicked){
                                    if(window.getAction()!=null){
                                        final DesktopNotify w=window;
                                        final long lf=l;
                                        invokeLater(new Runnable(){@Override public void run(){
                                            w.getAction().actionPerformed(new ActionEvent(w, ACTION_PERFORMED, "fireAction", lf, 0));
                                        }});
                                    }if(window.expTime()==MAX_VALUE){
